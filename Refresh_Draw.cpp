...
UserInterface::UserInterface() {
	...
	swidth_inital_life = (SCREEN_WIDTH * 1 / 32) / getSpriteWidth(life);
	sheight_initial_life = (SCREEN_HEIGHT * 1 / 12) / getSpriteHeight(life);
	...
	swidth_controls = (SCREEN_WIDTH * 1 / 8) / getSpriteWidth(controlsUI_sound);
	sheight_controls = (SCREEN_HEIGHT * 2 / 3) / getSpriteHeight(controlsUI_sound);
...
{
	float life_width = getSpriteWidth(life) * swidth_inital_life;

	//DRAW LIVES
	drawSprite(livesUI, UI_LIVES_POS_X, UI_LIVES_POS_Y, swidth_UI_lives, sheight_UI_lives);

	for (int i = 0; i < numLives; i++) {
		drawSprite(life, UI_LIVES_POS_X + i * life_width, UI_LIVES_POS_Y, swidth_inital_life, sheight_initial_life);
	}
...
void UserInterface::refreshScoreboard(const playerArray & p)
{
	...
	{
		drawFont(font3, al_map_rgb(0, 0, 0), TOPSCORES_POS_X, TOPSCORES_POS_Y + i*TOPSCORES_JUMP_Y, ALLIGN_LEFT, std::to_string(i + 1) + ". " + p[i].name);
		drawFont(font3, al_map_rgb(0, 0, 0), TOPSCORES_SCORE_POS_X, TOPSCORES_SCORE_POS_Y + i*TOPSCORES_JUMP_Y, ALLIGN_RIGHT, std::to_string(p[i].score.ReturnScore()));
	}
}
...
void UserInterface::SetReset(bool option) {
	reset = option;
}
...
void UserInterface::SetKeyAudio() {
	if (key_audio) key_audio = false;
	else {
		key_audio = true;
	}
	if (!key_audio) {
...
void UserInterface::SetSubLiveReset(bool option) {
	sub_live_reset = option;
}
...
bool UserInterface::GetPause() const {
	return pause;
}

bool UserInterface::GetGameOver() const {
	return game_over;
}

bool UserInterface::GetAuxGameOver() const {
	return aux_gameover;
}

bool UserInterface::GetReset() const {
	return reset;
}

bool UserInterface::GetAudio() const {
	return audio;
}
...
Score UserInterface::GetScore() const {
	return score;
}
...
int UserInterface::GetResetCounter() const {
	return reset_counter;
}

ALLEGRO_SAMPLE_INSTANCE * UserInterface::GetpacmanSongInstance() const {
	return songInstance;
}

ALLEGRO_SAMPLE * UserInterface::GetpacmanFXSound() const {
	return pacmanFX;
}
...
UserInterface::~UserInterface() {
	...
	destroySprite(controlsUI_sound);
	destroySprite(controlsUI_muted);
}
