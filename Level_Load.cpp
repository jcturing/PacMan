...
tGame_Element Level::CharacterTransform(const char& c) {
	...
	case 'p': aux = INTERIOR_WALL;
		break;
	case 'B': aux = BLUE_GHOST;
		break;
	case 'R': aux = RED_GHOST;
		break;
	case 'O': aux = ORANGE_GHOST;
		break;
	case 'K': aux = PINK_GHOST;
		break;
	case 'u': aux = CHERRY;
		break;
...
void Level::ResetBonusCounter() {
	bonus_counter = 0;
}

void Level::SetBonusCounter(int i) {
	bonus_counter = i;
}
...
void Level::RefreshBonus() {
	...
	if (bonus_counter >= bonus_time) {
		...
		if (!(bonus_pos[0] == 0 && bonus_pos[1] == 0)) {
			if (num >= 0 && num < 4) {
				SetMapElem(bonus_pos[1], bonus_pos[0], CHERRY);
			}
			else if (num >= 4 && num < 6) {
				SetMapElem(bonus_pos[1], bonus_pos[0], PINEAPPLE);
			}
			else {
				SetMapElem(bonus_pos[1], bonus_pos[0], WATERMELON);
			}
		}
		SetBonusCounter(0);
	}
...
float Level::GetPixelPositionX(int j) const {
	return map[row_max][j].pixel_position[0];
}
...
int Level::GetBonusCounter() const {
	return bonus_counter;
}

int Level::GetPacmanXPos() const
{
	return pacman_pos[0];
}

int Level::GetPacmanYPos() const
{
	return pacman_pos[1];
}

int Level::GetRedGhostXPos() const {
	return rg_pos[0];
}

int Level::GetRedGhostYPos() const {
	return rg_pos[1];
}

int Level::GetPinkGhostXPos() const {
	return pg_pos[0];
}

int Level::GetPinkGhostYPos() const {
	return pg_pos[1];
}

int Level::GetOrangeGhostXPos() const {
	return og_pos[0];
}

int Level::GetOrangeGhostYPos() const {
	return og_pos[1];
}

int Level::GetBlueGhostXPos() const {
	return bg_pos[0];
}

int Level::GetBlueGhostYPos() const {
	return bg_pos[1];
}

int Level::GetHouseXPos() const {
	return house_pos[0];
}

int	Level::GetHouseYPos() const {
	return house_pos[1];
}

int Level::GetNumMap() const {
	return num_map;
}
